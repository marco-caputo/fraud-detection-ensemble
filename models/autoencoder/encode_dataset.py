...
outlier_path = os.path.join(script_dir, "..", "..", DATASET_FOLDER_NAME, f"{OUTLIER_DATASET_NAME}.csv")
...
X_outlier_tensor = torch.tensor(outlier_dataset.iloc[:, :-1].values, dtype=torch.float32).to(device)
...
with torch.no_grad():
    ...
    outlier_encoded_data = model.encode(X_outlier_tensor).cpu().numpy()


print("Processing obtained dataset...")
...
outlier_encoded_df = pd.DataFrame(outlier_encoded_data, columns=[f"latent_{i}" for i in range(outlier_encoded_data.shape[1])])
...
outlier_encoded_df["label"] = outlier_dataset.iloc[:, -1].values
...
encoded_outlier_path = os.path.join(script_dir, "..", "..", DATASET_FOLDER_NAME, f"{ENCODED_OUTLIER_DATASET_NAME}.csv")
...
outlier_encoded_df.to_csv(encoded_outlier_path, index=False)
